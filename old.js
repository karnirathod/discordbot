...
client.on('message', msg => {
  ...
  if (msg.content.startsWith("!")) {
    ...
    // Initiating the password generation process with the "password" command
      ...
      msg.author.createDM()
        .then(channel => {
          channel.send(`Should your password have symbols?`)
    ...
    // Providing information about bot commands with the "help" command
      ...
      .then(link => {
        ...
        const embed = new Discord.MessageEmbed()
          ...
          .setDescription(`Password bot is the easiest and simplest Discord bot for everyone. The function of this discord bot is to make random passwords for you.\n\n[Click to invite me](${link})`)
          ...
          .addFields(
            ...
            { name: '`!ping`: Gives you a pong', value: '\u200b' },
            { name: '`!react`: Gives you a reaction', value: '\u200b' }
          )
          .setTimestamp()
          .setFooter('Password Bot - A Discord Bot by Karni (c) 2020 - 20XX');
...
class PasswordBuilder {
  ...
  generatePassword() {
    ...
    for (let i = 0; i < this.length; i++) {
      const symbolsOrCapitals = Math.random() > 0.3; // 70% chance of being letters
